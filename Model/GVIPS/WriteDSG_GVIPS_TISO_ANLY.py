def WriteDSG_GVIPS_TISO_ANLY(self, temp_dir, tests):
    #----------------------------------------------------------------------------------------------------------------------------------------------------------------
    #
    ...
    file.write("NDV:\n")
    NDV = 9*int(self.Compare['Analysis']['N'])
    file.write(' ' + str(NDV)+ '\n')
    ...
    # -- Save EL
    P_Elas = []
    val = float(self.Compare['Analysis']['VE_Param'][VE.index('EL')][2])
    unit = self.Compare['Analysis']['VE_Param'][VE.index('EL')][1]
    val = UnitConversion(unit, val, 'MPa')
    P_Elas.append(val)

    # -- Save ET
    val = float(self.Compare['Analysis']['VE_Param'][VE.index('ET')][2])
    unit = self.Compare['Analysis']['VE_Param'][VE.index('ET')][1]
    val = UnitConversion(unit, val, 'MPa')
    P_Elas.append(val)

    # -- Save vL
    val = float(self.Compare['Analysis']['VE_Param'][VE.index('νL')][2])
    P_Elas.append(val)

    # -- Save GL
    val = float(self.Compare['Analysis']['VE_Param'][VE.index('GL')][2])
    unit = self.Compare['Analysis']['VE_Param'][VE.index('GL')][1]
    val = UnitConversion(unit, val, 'MPa')
    P_Elas.append(val)
    ...
    # -- Write ξ
    val = float(self.Compare['Analysis']['VP_Param'][VP.index('ξ')][2])
    line = line + ' ' + str(val)
    Param.append('ξ')
    Param_V.append(val)
    Param_U.append('')
    ...
    # -- Write ζ
    val = float(self.Compare['Analysis']['VP_Param'][VP.index('ζ')][2])
    line = line + ' ' + str(val)
    Param.append('ζ')
    Param_V.append(val)
    Param_U.append('')
